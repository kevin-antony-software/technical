<x-admin.nav>
    <div class="container">
        <h2>Job Estimate Form</h2>
        <form method="POST" action="{{ route('repair_job.estimateSave', $job->id) }}">
            @csrf
            <div class="row">
                <div class="col-lg-12">
                    <style>
                                    ...
                                    @for ($i = 1; $i < 30; $i++)
                                        <tr>
                                            <td><input type="text" name="itemNo_{!! $i !!}" readonly
                                                    ...
                                                    autocomplete="off"
                                                    @isset($jobDetails[$i - 1]->component_id) value="{{ $jobDetails[$i - 1]->component_id }}" @endisset>
                                            </td>
                                            <td><input type="number" name="itemPrice_{!! $i !!}"
                                                    id="itemPrice_{!! $i !!}" class="form-control" readonly
                                                    autocomplete="off"
                                                    @isset($jobDetails[$i - 1]->component_price) value="{{ $jobDetails[$i - 1]->component_price }}" @endisset>
                                            </td>
                                            <td><input type="text" name="itemName_{!! $i !!}"
                                                    id="itemName_{!! $i !!}" class="form-control"
                                                    autocomplete="off"
                                                    @isset($jobDetails[$i - 1]->component_id) value="{{ $jobDetails[$i - 1]->component_name }}" @endisset>
                                            </td>
                                            <td><input type="text" name="quantity_{!! $i !!}"
                                                    ...
                                                    accesskey="a"
                                                    @isset($jobDetails[$i - 1]->component_id) value="{{ $jobDetails[$i - 1]->qty }}" @endisset>
                                            </td>
                                        </tr>
                                    @endfor
                                </tbody>
    ...
    <div class="container pt-5">
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="number" id="repairCharges" name="repairCharges"
                value="{{ $job->repairCharges }}">
        </div>
        <div class="form-group">
            <label for="totalCharges">Total Charges</label>
            <input class="form-control" type="number" id="totalCharges" name="totalCharges"
                value="{{ $job->totalCharges }}">
        </div>
        <div class="form-group">
            <label for="discount">Discount</label>
            <input class="form-control" type="number" id="discount" name="discount" value="{{ $job->discount }}">
        </div>
        <div class="form-group">
            <label for="finalTotal">Final Total</label>
            <input class="form-control" type="number" id="finalTotal" name="finalTotal" value="{{ $job->finalTotal }}">
        </div>
        ...
        <div id="not-common-area">
            <div class="form-group">
                <h3> Actual Issue of the Machine </h3>
        ...
        <div class="container pt-5">
            <div class="row">
                ...
                <div class="col-6">
                    <button type="submit" class="btn btn-block btn-primary">Estimate Repair Job</button>
                </div>
            </div>
        </div>
        </form>
